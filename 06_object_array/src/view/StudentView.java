...
public class StudentView {
	...
	private void allStudent() {
		...
		for(StudentDTO std : students) {
			...
			if(std == null) {
				// null.toString() : 오류가 발생하기 때문에 오류 구문 수행 전 검증
				continue;				// 다음 반복으로 넘어감
			}
			System.out.println(std.toString());
			/* 참조 변수를 문자열 만들 때 또는 print() 관련 메서드에 사용될 때
			 * 참조 변수에 저장된 주소가 출력되는게 아니라
			 * 컴파일러가 자동으로 참조변수.toString()을 호출하도록 변경한다 
			 */
		}
	}
	 ...
	 * -> 추가 성공 시 true, 실패 시 false를 반환받아 결과 출력
	 * </pre>
	 */
	private void addStudent() {
		...
		// 생성된 StudentDTO 객체를
		// StudentService.addStudent() 메서드에 전달
		boolean result = service.addStudent(student);		// true/false 결과 반환 예정
	 ...
	 * <pre>
	 * 인덱스를 입력받아 StudentService.selectIndex()로 전달
	 * 학생 객체 배열에서 해당 index번째 학생을 반환
	 * 반환받은 학생 정보를 출력
	 ...
	 * 
	 * 인덱스가 정상 입력된 경우
	 * : 해당 index번째 학생(StudentDTO)을 반환받기
	 * 	 합계,평균 구하기
	 * 	 아래와 같은 형식으로 출력하기
	 * 	 ------------------------------------------
			 [20250001] 짱구(남) 
