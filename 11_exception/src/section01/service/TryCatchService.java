...
public class TryCatchService {
	...
	*/
	 ...
	 * catch (예외 참조 변수){ }
	 * : try에서 던져진 예외를 ()내의 예외 참조변수가 잡아서 처리하는 구문
	 * : 잡아서 처리하면 예외가 사라짐
	 * : 프로그램이 종료되지 않고 계속 정상 수행
	 * </pre>
	 */
	public void test1() {
		// 입력받은 두 정수를 나눈 몫 계산하기
		...
		try {
			...
			// try에서 ArithmeticException 객체가 던져지면 매개변수 e를 이용해서 잡는다
	...
	public void test3() {
		...
		try {
			...
			// ArithmeticException과 InputMismatchException의 공통 부모 타입을 매개 변수로 선언
			// try에서 예외 발생 시 "예외 객체 참조 주소"가 던져짐
			// 업 캐스팅 : 부모타입 참조변수 = 자식객체 참조주소;
			// 부모타입 예외 참조 변수로 던져지는 모든 자식 예외를 잡겠다
	...
	public void test4() {
		...
		try {
			...
			/*
				'부모 타입 예외 처리 구문'을 먼저 작성하면
				'자식 타입 예외 처리 구문'까지 모두 잡기 때문에
	...
	/* finally{}
		: try 구문에서 예외 발생 여부 관계 없이 마지막에 무조건 실행되어야하는 코드를 작성하는 구문
		: 중간에 프로그램이 종료되거나 메서드가 리턴되어도 무조건 실행된다
		: 외부 프로그램/장치와의 연결을 끊어낼 때 주로 사용한다
	*/
	public void test6() {
