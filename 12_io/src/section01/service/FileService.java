...
public class FileService {
	...
	*/
	...
	/* [개발 상식]
		...
		Windows		)		\			ex) C:\workspace\04_Java
		Linux/Mac )		/			ex) /users/OOO/workspace/04_Java
		# Java는 모든 운영체제에서 호환된다
			-> 코드에서 경로 표기시 \,/를 신경쓰지 않아도 된다
			-> JVM이 실행 시 실행되는 운영체제에 맞춰서 해석한다
	...
	*/
	...
	public void test1() {
		...
		if(!directory.exists()) {						// 폴더가 없다면
			...
			// 부모 폴더 삭제 시 자식 폴더/파일이 있으면 삭제 불가
				// -> 내부 폴더/파일을 모두 지우고 부모 폴더 삭제
			if(parent.delete()) {
	...
	public void test2() {
		/*
			절대 경로
				절대적 기준(root 폴더)부터 경로 표기
			상대 경로
				현재 위치/프로젝트 기준으로 경로 표기
		*/
		// File 클래스를 이용해 객체 생성
		...
		if(!file.exists()) {								// 파일생성.txt 파일이 없다면
			try {
				// 파일 생성
	...
	public void test3() {
		...
		for(File f : files) {
			...
			long lastModified = f.lastModified();
				// 1970.01.01 09:00:00.000 기준 지난 시간만큼의 ms 값이 반환
			...
			if(f.isFile()) {									// 파일인 경우에만 크기 측정
				long fileSize = f.length();			// 파일의 Byte 크기 반환
