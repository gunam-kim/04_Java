...
public class Student {
	 ...
	 * 	 유연성을 확보하는 것 
	 */
	private String studentNumber;						// private 접근 제한자 : 외부 직접 접근을 제한
	private String name;
	private int html;
	private int css;
	private int js;
  ...
  */
	...
	/** 기본 생성자
	 * - 매개변수가 없는 생성자
	 * - 클래스 내에 생성자가 1개도 없으면 컴파일러가 자동으로 추가
	 */
	public Student() {
		System.out.println("! Student 객체가 기본 생성자로 생성되었습니다 !");
		
		// 객체가 Heap영역에 생성될 때 필드값이 모두 JVM 초기값으로 세팅이 진행됨
		// (Heap 영역은 비어있는 상태로 존재 불가)
		// (정수형 : 0, 실수형 : 0.0, 논리형 : false,
		// 문자형 : '\u0000', 참조형 : null (참조하는 객체가 없다))
		
		/* 생성자를 이용해서 객체가 생성될 때 필드값을 원하는 값으로 초기화할 수 있다 */
	...
	public Student(String studentNumber, String name) {
		System.out.println("! Student(String, String) 생성자로 생성되었습니다 !");
	...
	public Student(String studentNumber, String name,
	 int html, int css, int js, int java) {
		System.out.println("! 모든 필드를 초기화하는 생성자로 생성되었습니다 !");
		this.studentNumber = studentNumber;
		this.name = name;
		this.html = html;
		this.css = css;
	 ...
	 *   매개변수(순서,타입,개수)를 다르게하여 같은 이름의 메서드라도
	 *   구분할 수 있게 한다
	 */
	...
	public String toString() {
		...
		String str
			= String.format("[%s] %s\n"
				...
				+ "합계 : %d\n" + "평균 : %.2f",
				studentNumber, name, html, css, js, java,
				getSum(), getAverage());
