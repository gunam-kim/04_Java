...
- abstract 예약어 작성
- 추상 클래스는 보통 미완성 부분(추상 메서드)을 가지고 있어 객체 생성이 불가능하다
- 직접 객체 생성은 불가능하지만, 자식 클래스에서 상속받아 미완성 부분(추상 메서드)을 정의해서
	완성 시켜주면 자식 객체는 생성 가능하다
	(오버라이딩 강제화, 동적 바인딩)
- 추상 클래스도 생성자는 필수로 존재해야만 한다
	(상속받은 자식 내부에 부모 부분이 생성되어야 되기 때문에)
