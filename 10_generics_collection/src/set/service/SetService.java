...
public class SetService {
	/* Set (집합)
		...
		: 중복 데이터(객체) 저장 X
		: 중복된 객체가 들어오면 덮어쓰기
		: (중요!) equals(), hashCode() 오버라이딩 필수 (동등 비교를 하기위해)
	...
	*/
	...
	public void test1() {
		// HashSet 객체 생성
		Set<String> set = new HashSet<String>();				// 인터페이스 set = 구현체 HashSet
		// String은 equals(),hashCode() 오버라이딩된 상태
		...
		set.add(null);
		set.add(null);
		System.out.println(set);									// null도 중복 허용 X
	...
	/**
	 * Hash 단어가 포함된 컬렉션은 무조건 equals(),hashCode()가 오버라이딩되어있어야 한다!
	 * 
	 * @@@ Object가 제공하는 메서드 오버라이딩 목적 @@@
	 * equals() : 객체의 필드가 같은지 비교 (동등 비교)
	 * hashCode() : 객체의 식별번호. equals()가 true이면 hashCode()도 같아야함
	 */
	public void test2() {
		...
		// LinkedHashSet 객체 생성
		Set<Person> personSet = new LinkedHashSet<Person>();	// 순서 유지됨
